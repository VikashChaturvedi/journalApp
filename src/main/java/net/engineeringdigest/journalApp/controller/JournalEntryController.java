...
public class JournalEntryController {
    ...
    public ResponseEntity<JournalEntry> getJournalEntryById(@PathVariable ObjectId myId){
        ...
        if(!collect.isEmpty()){
            Optional<JournalEntry> journalEntry=journalEntryService.searchById(myId);
    ...
    public ResponseEntity<?> updateJournalById(@PathVariable ObjectId myId,
        ...
        List<JournalEntry> collect = user.getJournalEntries().stream().filter(x-> x.getId().equals(myId)).collect(Collectors.toList());
        if(!collect.isEmpty()){
            //below journalEntry is of type optional so we are converting it into journal entry type to update in DB.
            Optional<JournalEntry> journalEntry=journalEntryService.searchById(myId);
            if(journalEntry.isPresent()) {
